...
figHandle = y.show(subplot=[2,9,1])
...
exp.setData(export.PLOTHANDLE, figHandle)
exp.doExport()
...
figHandle = fft.show(y_hat_ideal, f_p, subplot=[2,9,10], title="FFT (ref)")
...
while mrot <= PI/2:
    ...
    figHandle = qft.show(y_hat_real, f_p, subplot=[2,9,pt+3], title=f"QFT_sim, mr:{mrot:.2f}", xlabel=" ", ylabel=ylabel)
...
figHandle = grader_inst.show(subplot=[2,9,9])
...
while mrot <= PI/2:
    ...
    y_hat_real, f_p = qft.postProcess(y_hat, f)
    ylabel = "Amplitude" if pt == 0 else " "
    figHandle = qft.show(y_hat_real, f_p, subplot=[2,9,pt+12], title=f"QFT_real, mr:{mrot:.2f}",  xlabel="Freq (Hz)", ylabel=ylabel)
    snr = grader_inst.calculateNoisePower(y_hat_real, y_hat_ideal)
    print(f"Calculated an snr of {snr} db")
    grader_inst.log(snr, mrot)
    print(f"Minimum rotation is: {mrot}")
...
figHandle = grader_inst.show(subplot=[2,9,18])
