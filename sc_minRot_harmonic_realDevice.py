...
plotData = y.show(subplot=[rows,nQubits+3,1], title='signal')
...
plotData = fft.show(y_hat_ideal_p, f_p, subplot=[rows,nQubits+3,nQubits+4], title="FFT (ref)")
...
for expIt in range(0, nExp):
    ...
    while mrot <= PI/2:
        ...
        plotData = qft.show(y_hat_sim_p, f_p, subplot=[rows,nQubits+3,pt+2], title=f"QFT_sim, mr:{mrot:.2f}", xlabel=" ", ylabel=ylabel)
...
for expIt in range(0, nExp):
    ...
    while mrot <= PI/2:
        qft = transform(qft_framework, minRotation=mrot, suppressPrint=False, simulation=True, useNoiseModel=True, backend=device)

        y_hat, f = qft.forward(y)
        y_hat_sim_n_p, f_p = qft.postProcess(y_hat, f)
        ylabel = "Amplitude" if pt == 0 else " "
        # 2nd entry in 2nd row: 0+1+(nQubits+3)+1)
        plotData = qft.show(y_hat_sim_n_p, f_p, subplot=[rows,nQubits+3,pt+nQubits+5], title=f"QFT_sim_n, mr:{mrot:.2f}",  xlabel="Freq (Hz)", ylabel=ylabel)
...
for expIt in range(0, nExp):
    ...
    while mrot <= PI/2:
        ...
        ylabel = "Amplitude" if pt == 0 else " "
        # 2nd entry in 2nd row: 0+1+(nQubits+3)+1)
        plotData = qft.show(y_hat_real_p, f_p, subplot=[rows,nQubits+3,pt+2*nQubits+8], title=f"QFT_real, mr:{mrot:.2f}",  xlabel="Freq (Hz)", ylabel=ylabel)
    ...
    plotData = grader_inst.show(subplot=[rows,nQubits+3,3*(nQubits+3)])
